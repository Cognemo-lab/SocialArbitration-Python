...
IN_CSV = BASE / "hgf_baselinefixed_full" / "construct_validity_behavior" / "behavior_session_summary.csv"
OUT_DIR = BASE / "behavior_test_retest_comparison" / "original_intra_behavior"
...
def add_early_late_accuracy(df: pd.DataFrame) -> pd.DataFrame:
    wide = pd.read_csv(BASE / "behavior_test_retest_comparison" / "original_behavior_t1_t2_wide.csv")
    rows = []
    for tp in ["t1", "t2"]:
        tmp = wide[
            ["prolific_id", f"accuracy_first30_{tp}", f"accuracy_last20_{tp}"]
        ].rename(
            columns={
                f"accuracy_first30_{tp}": "accuracy_first30",
                f"accuracy_last20_{tp}": "accuracy_last20",
            }
        )
        tmp["timepoint"] = tp
        rows.append(tmp)
    extra = pd.concat(rows, ignore_index=True)
    return df.merge(extra, on=["prolific_id", "timepoint"], how="left")
...
def main() -> None:
    ...
    df = pd.read_csv(IN_CSV)[["prolific_id", "timepoint", "accuracy_rate", "advice_taking_rate", "win_stay_rate", "lose_switch_rate", "mean_wager"]].copy()
    df = add_early_late_accuracy(df)
    ...
    fig.suptitle("Original sample intra-behavioral correlations at T1 and T2", fontsize=16, y=0.98)
