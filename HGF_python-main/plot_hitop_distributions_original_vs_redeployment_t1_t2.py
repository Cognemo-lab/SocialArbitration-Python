...
OUT_FIG = OUT_DIR / "figure_hitop_distributions_original_vs_redeployment_t1_t2.png"
OUT_CSV = OUT_DIR / "hitop_distributions_original_vs_redeployment_t1_t2_summary.csv"
...
def load_long() -> pd.DataFrame:
    orig_t1 = pd.read_csv(BASE / "hitop" / "processed_data" / "hitop_scales_T1.csv")[
        ["prolific_id", *[m for m, _ in METRICS]]
    ].copy()
    orig_t1["dataset"] = "Original"
    orig_t1["timepoint"] = "t1"

    orig_t2 = pd.read_csv(BASE / "hitop" / "processed_data" / "hitop_scales_T2.csv")[
        ["prolific_id", *[m for m, _ in METRICS]]
    ].copy()
    ...
    df = pd.concat([orig_t1, orig_t2, red_t1, red_t2], ignore_index=True)
    return df
...
def main() -> None:
    ...
    for ax, (metric, label) in zip(axes, METRICS):
        ...
        colors = []
        ns = []
        for dataset, timepoint, group_label, color in GROUPS:
            s = pd.to_numeric(
                df.loc[(df["dataset"] == dataset) & (df["timepoint"] == timepoint), metric],
                errors="coerce",
            ).dropna()
        ...
        ax.set_xticklabels([g[2] for g in GROUPS], rotation=0, fontsize=10)
        ax.grid(axis="y", alpha=0.18)
        ax.set_ylabel("Score")

    fig.suptitle("HiTOP measure distributions across original and redeployed samples", fontsize=18, y=0.995)
